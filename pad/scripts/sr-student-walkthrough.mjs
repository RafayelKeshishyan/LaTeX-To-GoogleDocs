...
async function focusInfo(page) {
  return page.evaluate(() => {
    ...
    if (!el || el === document.body) return { tag: 'body', id: '', name: '', role: '' };
    const label =
...
async function reviewSelection(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      focusInList: document.activeElement === list,
...
async function main() {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.removeItem('digimath-pad-linear-tip-v1');
  });
  ...
  const earlyNavigationOrder = await page.evaluate(() => {
    ...
    return {
      ...
      currentClass: all.indexOf(document.querySelector('.sheet-navigation select')),
      currentPage: all.indexOf(document.querySelectorAll('.sheet-navigation select')[1]),
      linear: all.indexOf(document.querySelector('.linear-editor textarea')),
    };
  });
  if (
    !(
  ...
  const keyboardHelpState = await page.evaluate(() => {
    ...
    const editor = document.querySelector('.linear-editor textarea');
    return {
  ...
  f = await focusInfo(page);
  if (!/Close help and return/i.test(f.name || f.text || '')) {
    issue(`Tab after topics should reach bottom Close, got ${JSON.stringify(f)}`);
  }
  await page.keyboard.press('Tab');
  await page.waitForTimeout(50);
  ...
  const linearAttributes = await page.locator('.linear-editor textarea').evaluate((el) => ({
  ...
  if (
    linearAttributes.label !== 'Equation 1 of 1' ||
    ...
    issue(`Linear field should speak only Equation N of M: ${JSON.stringify(linearAttributes)}`);
  } else {
    note('Linear field speaks Equation N of M without LaTeX jargon');
  }

  await page.waitForTimeout(800);
  const linearTipStatus = await statusText(page);
  if (!/Tip:.*Alt\+Enter hears math.*Alt\+Up or Alt\+Down changes equation/i.test(linearTipStatus || '')) {
    issue(`First Linear focus should give a one-time tip: ${linearTipStatus}`);
  } else {
    note('First Linear focus announces a one-time Alt+Enter / Alt+Up tip');
  }
  const tipStored = await page.evaluate(() => localStorage.getItem('digimath-pad-linear-tip-v1'));
  if (tipStored !== '1') {
    issue('Linear tip should be remembered so it only plays once');
  } else {
    note('Linear tip is stored so it only plays once');
  }

  const linearFocusStyle = await page.locator('.linear-editor textarea').evaluate((el) => {
  ...
  await page.waitForTimeout(100);
  f = await focusInfo(page);
  if (f.tag !== 'textarea') issue(`Escape back failed: ${JSON.stringify(f)}`);
  ...
  await page.keyboard.press('Enter');
  await page.waitForTimeout(50);
  await page.locator('.linear-editor textarea').focus();

  // Invalid LaTeX should stay attached to Linear and block a ready-to-submit result.
  await page.locator('.linear-editor textarea').fill('\\sqrt{');
  await page.keyboard.press('Alt+Enter');
  await page.waitForTimeout(100);
  const invalidState = await page.locator('.linear-editor textarea').evaluate((el) => ({
  ...
  await openDetails(page, 'Export and backup');
  await page.getByRole('button', { name: /Copy all answers/i }).click();
  await page.waitForTimeout(100);
  f = await focusInfo(page);
  const invalidReview = await reviewSelection(page);
  if (f.role !== 'listbox' || !/Equation 1/i.test(invalidReview.selectedLabel || '')) {
    issue(
  ...
  await page.waitForTimeout(100);
  const returnedError = await page.locator('.linear-editor textarea').evaluate((el) =>
  ...
  await page.locator('.linear-editor textarea').fill('y=\\sqrt{x+3}');
  ...
  if (f.name !== 'Equation 2 of 2' || newEquationStatus?.trim()) {
  ...
  const workListTab = await page.locator('#work-list').evaluate((el) => ({
    ...
    role: el.getAttribute('role'),
    optionTabs: Array.from(el.querySelectorAll('[role="option"]')).filter((row) => row.tabIndex === 0)
      .length,
  }));
  if (workListTab.tabIndex !== 0 || workListTab.role !== 'listbox' || workListTab.optionTabs !== 0) {
    issue(`Your work should be one listbox Tab stop, found ${JSON.stringify(workListTab)}`);
  } else {
    note('Your work uses one listbox Tab stop');
  }
  ...
  await page.keyboard.press('Enter');
  await page.waitForTimeout(100);
  const latex1 = await page.locator('.linear-editor textarea').inputValue();
  ...
  await page.waitForTimeout(50);
  const labeledEquation = await page.locator('.linear-editor textarea').getAttribute('aria-label');
  const labeledRow = await page.locator('#work-list [aria-selected="true"]').getAttribute('aria-label');
  ...
  const noteCount = await page.locator('#work-list .block-select').evaluateAll((rows) =>
    rows.filter((row) => /^Note\b/i.test(row.getAttribute('aria-label') || '')).length,
  );
  ...
  await page.locator('#work-list').focus();
  await page.keyboard.press('Home');
  await page.keyboard.press('Enter');
  await page.waitForTimeout(80);

  await page.locator('.linear-editor textarea').evaluate((el) => {
    const field = el;
    field.focus();
  ...
  await page.waitForTimeout(80);
  let latexAfterPlainDown = await page.locator('.linear-editor textarea').inputValue();
  if (!latexAfterPlainDown.includes('sqrt')) {
    issue(`Plain ArrowDown in Linear should stay on the current equation, got ${latexAfterPlainDown}`);
  } else {
    note('Plain ArrowDown in Linear only edits and does not change equations');
  }
  await page.keyboard.press('Alt+ArrowDown');
  await page.waitForTimeout(100);
  f = await focusInfo(page);
  const latexAfterDown = await page.locator('.linear-editor textarea').inputValue();
  if (f.tag !== 'textarea' || !latexAfterDown.includes('x^2')) {
    issue(
      `Alt+ArrowDown from Linear should open the next equation, got ${JSON.stringify({
        focus: f,
        latexAfterDown,
      })}`,
    );
  } else {
    note('Alt+ArrowDown moves to the next equation');
  }
  await page.keyboard.press('Alt+ArrowUp');
  await page.waitForTimeout(100);
  const latexAfterUp = await page.locator('.linear-editor textarea').inputValue();
  if (!latexAfterUp.includes('sqrt')) {
    issue(`Alt+ArrowUp from Linear should return to the previous equation, got ${latexAfterUp}`);
  } else {
    note('Alt+ArrowUp moves to the previous equation');
  }

  // List arrows keep focus
  await workList.focus();
  await page.keyboard.press('ArrowDown');
  await page.waitForTimeout(80);
  f = await focusInfo(page);
  if (f.tag === 'textarea') issue('Arrow stole focus into Linear');
  else note('Arrow on list keeps list focus');

  // Delete from the selected Your work row with confirmation.
  await workList.focus();
  await page.keyboard.press('Home');
  await page.waitForTimeout(80);
  const homeRow = await page.locator('#work-list [aria-selected="true"]').getAttribute('aria-label');
  if (!/^1\.2\b/.test(homeRow || '')) {
    issue(`Home should select problem 1.2 before delete, got ${homeRow}`);
  }
  await workList.focus();
  await page.keyboard.press('Delete');
  const removeDialog = page.getByRole('alertdialog', { name: /Remove /i });
  if (!(await removeDialog.isVisible())) {
    issue('Removing a completed equation did not open the accessible confirmation dialog');
  } else {
    note('Removing a completed equation opens an accessible confirmation dialog');
  }
  ...
  if (!confirmFocus.inDialog) {
    issue(`Confirmation dialog did not take focus, got ${JSON.stringify(confirmFocus)}`);
  } else {
    note('Confirmation dialog receives keyboard focus');
  }
  const countBeforeBlockedShortcut = await page.locator('.doc-pane .block-select').count();
  await page.keyboard.press('Alt+=');
  const countAfterBlockedShortcut = await page.locator('.doc-pane .block-select').count();
  if (countAfterBlockedShortcut !== countBeforeBlockedShortcut) {
    issue('Alt+= changed work behind the confirmation dialog');
  } else {
    note('Global equation shortcuts are blocked behind dialogs');
  }
  await page.keyboard.press('Escape');
  await page.waitForTimeout(80);
  f = await focusInfo(page);
  if ((await removeDialog.isVisible()) || f.id !== 'work-list') {
    issue(`Cancelling confirmation should restore the opener, got ${JSON.stringify(f)}`);
  } else {
    note('Escape cancels confirmation and restores its opener');
  }
  await page.keyboard.press('Delete');
  ...
  // Undo restore (app-level; works even if Linear is focused)
  ...
  if (
    ...
    reviewSemantics.name !== 'review-heading' ||
    reviewSemantics.description !== 'review-summary'
  ) {
    issue(`Review is not an open, named modal dialog: ${JSON.stringify(reviewSemantics)}`);
  } else {
    note('Review is an open, named and described modal dialog');
  }
  const reviewIds = await page.evaluate(() => {
    const workIds = [...document.querySelectorAll('#work-list [role="option"]')].map((el) => el.id);
    const reviewOpts = [...document.querySelectorAll('.review-list [role="option"]')].map((el) => ({
      id: el.id,
      selected: el.getAttribute('aria-selected') === 'true',
    }));
    const active = document.querySelector('.review-list')?.getAttribute('aria-activedescendant') || '';
    const selected = reviewOpts.find((opt) => opt.selected);
    return {
      duplicateAcrossLists: workIds.some((id) => reviewOpts.some((opt) => opt.id === id)),
      activeMatchesSelected: Boolean(selected && active === selected.id),
  ...
  if (f.role !== 'listbox' || !/1\.2|Equation|Note/i.test(selection.selectedLabel || '')) {
  ...
  const reviewTabStops = await page.locator('.review-list').evaluate((list) => ({
    ...
    optionTabStops: [...list.querySelectorAll('[role="option"]')].filter((row) => row.tabIndex >= 0)
      .length,
  }));
  if (reviewTabStops.listTabIndex !== 0 || reviewTabStops.optionTabStops !== 0) {
    issue(`Review should use one listbox Tab stop, got ${JSON.stringify(reviewTabStops)}`);
  } else {
    note('Review rows use one listbox Tab stop');
  }
  ...
  await page.locator('.review-list').focus();
  ...
  if (!/Review answers/i.test(f.text || f.name || '')) {
    issue(`Closing Review should restore focus to Review answers, got ${JSON.stringify(f)}`);
  } else {
    note('Closing Review restores focus to Review answers');
  }
  ...
  await page.waitForTimeout(80);
  f = await focusInfo(page);
  if (f.tag !== 'textarea' || f.id === 'prose-focus-target') {
  ...
  // Copy all — may confirm empties
  await page.evaluate(async () => {
    // seed clipboard permission in chromium often works
  });
  ...
  if (!clip.includes('Equation') && !clip.includes('x^2') && !clip.includes('sqrt')) {
    // clipboard may be blocked in headless — check status instead
  ...
  const equationBeforeReload = await page.locator('.linear-editor textarea').getAttribute('aria-label');
  await page.waitForTimeout(350);
  ...
  const equationAfterReload = await page.locator('.linear-editor textarea').getAttribute('aria-label');
  ...
  // Import path: inject a sheet via file input
  ...
  const importedLatex = await page.locator('.linear-editor textarea').inputValue();
  if (!importedLatex.includes('a^2')) {
    // may need to focus equation
    const st = await statusText(page);
  ...
  await page.waitForTimeout(100);
  f = await focusInfo(page);
  if (f.tag !== 'textarea') issue(`Skip link failed: ${JSON.stringify(f)}`);
  else note('Skip link focuses Linear');
  ...
  const shortcutEditor = shortcutPage.locator('.linear-editor textarea');
