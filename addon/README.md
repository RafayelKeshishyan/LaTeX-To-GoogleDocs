...
- If Chrome extension version 2.12.0 is permitted, **Alt+Shift+E** returns focus from Docs to the already-open add-on sidebar. When the cursor is next to an add-on equation, its LaTeX loads for editing; otherwise the field stays in new-equation mode. Without the helper extension, Google Docs provides no dependable shortcut into a custom Apps Script sidebar.
