...
- If Chrome extension version 2.12.1 is permitted, **Ctrl+Shift+9** returns focus from Docs to the already-open add-on sidebar. When the cursor is next to an add-on equation, its natural wording is announced and its LaTeX loads selected for editing; otherwise the field stays in new-equation mode. `Alt+Shift+E` conflicts with the Google Docs Extensions menu. Without the helper extension, Google Docs provides no dependable shortcut into a custom Apps Script sidebar.
