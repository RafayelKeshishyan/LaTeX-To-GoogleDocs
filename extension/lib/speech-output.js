...
const SpeechOutput = (() => {
   ...
   */
  function prepareLiveAnnouncement(text, options = {}) {
    const message = (text || '').trim();
    const fallback = (options.fallback || '').trim();

    // MathML can be announced as only a fragment while focus is moving from
    // the iframe back to Google Docs. Prefer the complete natural-language
    // reading already generated by the extension. Professional Preview keeps
    // its MathML for direct exploration.
    const useNaturalSpeech = Boolean(fallback) && options.preferMathML !== true;
    const mathml =
      !useNaturalSpeech && options.latex && typeof LatexMathML !== 'undefined'
        ? LatexMathML.fromLatex(options.latex)
        : '';
    const prefix = useNaturalSpeech
      ? fallback
      : options.latex && !mathml
        ? (fallback || message)
        : message;

    return { prefix, mathml };
  }
  ...
  function announceInRegions(els, text, options = {}) {
    ...
    const { prefix, mathml } = prepareLiveAnnouncement(text, options);
